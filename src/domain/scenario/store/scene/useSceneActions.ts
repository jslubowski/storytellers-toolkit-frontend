...
import { sceneApi } from '@/domain/scenario/api/sceneApi.ts'
...
export function useSceneActions(
  ...
  const saveDescription = useDebounceFn(async (scenarioId: string, sceneId: string, description: string) => {
  ...
  const saveGmNotes = useDebounceFn(async (scenarioId: string, sceneId: string, gmNotes: string | null) => {
  ...
  return { fetchScene, saveDescription, saveGmNotes, isSavingDescription, isSavingGmNotes };
}
